...
public class ReusableMethods {
    ...
    public static void verifyAllItemsEqual(String elementText) {
        AndroidDriver driver = (AndroidDriver) Driver.getAndroidDriver();

        // Scroll through the list to find all elements matching elementText
        List<WebElement> elements = driver.findElementsByAndroidUIAutomator("new UiScrollable(new UiSelector()).scrollIntoView(text(\"" + elementText + "\"))");

        // Verify that each element's text matches elementText
        for (WebElement element : elements) {
            String text = element.getText();
            System.out.println(text);
            if (!text.contains(elementText)) {
                throw new AssertionError("Element text does not match expected value: " + text);
            }
        }
    }
